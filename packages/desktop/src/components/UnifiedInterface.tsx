...
export const UnifiedInterface: React.FC<UnifiedInterfaceProps> = React.memo(({
  ...
  // Resizable editor area height (children contains EditorArea)
  const [editorAreaHeight, setEditorAreaHeight] = useState<number>(() => {
    return Math.floor(window.innerHeight * 0.4);
  });

  const handleEditorResize = useCallback((newY: number) => {
    setEditorAreaHeight(newY);
  }, []);
  ...
  return (
    <ErrorBoundary onError={handleComponentError} resetKeys={[state.applicationMode]} resetOnPropsChange={true}>
      <div
        ...
        style={{ '--editor-area-height': `${editorAreaHeight}px` } as React.CSSProperties}
      >
        {/* Render children (includes toolbar and editor-area from UnifiedRecorderScreen) */}
        {children}

        {/* Resizable Divider between editor-area and data-area */}
        <ResizableDivider
          onResize={handleEditorResize}
          currentY={editorAreaHeight}
          minY={50}
          maxY={window.innerHeight - 58}
        />

        {/* Bottom section: data-area + TabBar (takes remaining space) */}
