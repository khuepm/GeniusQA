# Task 17.2: Test Rust-recorded Scripts with Python Playback

## Overview
This document provides a manual testing guide for validating that scripts recorded with the Rust core can be successfully played back with the Python core.
...
### Step 1: Record a Script with Rust Core
...
3. **Record a Test Script**
   ...
   - Save the script with name: `rust_recorded_test_1`
...
### Step 3: Play Back Rust-Recorded Script with Python Core

1. **Load the Script**
   - In the Script Selector, find `rust_recorded_test_1`
   - Select the script
...
2. Switch to Python Core and play back
...
1. **Test Pause/Resume**
   - Start playback of Rust-recorded script with Python core
...
1. **Test with Invalid Script**
   - Manually corrupt a Rust-recorded script JSON file
   - Try to play back with Python core
   - Verify error message is displayed
   - Verify UI returns to idle state

2. **Test with Missing Permissions**
   - Revoke automation permissions (if possible on your platform)
   - Try to play back Rust-recorded script with Python core
   - Verify permission error is displayed with helpful message

3. **Test with Out-of-Bounds Coordinates**
   - Manually edit a Rust-recorded script to have coordinates outside screen bounds
   - Play back with Python core
   - Verify coordinates are clamped and warning is logged

## Expected Results

### Compatibility
- ✅ All Rust-recorded scripts should load successfully in Python core
...
- [ ] Rust-recorded script loads in Python core
...
- **Solution**: Check JSON format is valid, verify file permissions, check for format differences

### Issue: Actions don't execute
- **Solution**: Check automation permissions, verify Python core is active, check PyAutoGUI installation
...
### Issue: Key actions fail
- **Solution**: Check if Rust uses different key naming conventions than Python
...
The following automated tests also validate Rust-to-Python compatibility:

- `packages/python-core/src/storage/test_rust_to_python_playback.py`
  - `test_load_rust_recorded_script`
  - `test_rust_mouse_actions_compatibility`
  - `test_rust_keyboard_actions_compatibility`
  - `test_rust_mixed_actions_compatibility`
...
This manual testing procedure validates that Rust-recorded scripts work correctly with Python playback, ensuring cross-core compatibility as specified in Requirement 10.2.
