...
- **AI Provider**: Dịch vụ AI bên ngoài cung cấp khả năng sinh script (Gemini, OpenAI, Anthropic, Azure)
- **Provider Adapter**: Module chuyển đổi request/response giữa hệ thống và từng provider cụ thể
...
- **Fallback Provider**: Provider dự phòng được sử dụng khi provider chính gặp lỗi
...
3. WHEN a user views configured providers THEN the System SHALL display which providers have valid API keys configured
...
**User Story:** As a user, I want to select which AI provider to use for script generation, so that I can choose the best AI for my specific needs.

#### Acceptance Criteria

1. WHEN a user opens the AI Script Builder THEN the System SHALL display a provider selector showing all configured providers
2. WHEN a user selects a provider THEN the System SHALL switch to using that provider for subsequent script generation
3. WHEN a user has only one provider configured THEN the System SHALL automatically select that provider
